...
void Ant::move() {
	...
	switch (random) {
	case 0: // Cell above
		if (basey == gridLen - 1) {//on right edge
		} else {
			if (isMove(x, y + 1)) {
				this->y++;
				return;
	...
	case 1: // Cell to right
		if (basex == gridLen - 1) {//on bottom edge
		} else {
			if (isMove(x + 1, y)) {
				this->x++;
				return;
	...
	//exact same ifs as doodlebug simplemove() check there for more info
	...
	if (basey == y && basex == x) {//didnt move but can
		move();
	}
}
